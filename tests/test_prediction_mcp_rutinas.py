...
class TestDeporteCambiaResultado:
    ...
    def test_predict_risk_deriva_nivel_del_met_antes_de_prediccion(self, monkeypatch):
        ...
        def _fake_try_prediction(lat, lon, provincia, perfil, target_date=None):
...
class TestRutinasMCP:
    def test_crear_listar_borrar(self, db):
    ...
    def test_crear_sin_identificador_error_claro(self, db):
        r = _json(mcp.crear_rutina_mcp(nombre="x", dias="1", hora_inicio=8.0, hora_fin=9.0))
        assert r.get("error") and "chat_id" in r["error"]

    def test_crear_perfil_sin_chat_error_claro(self, db):
    ...
    def test_crear_dias_invalidos(self, db):
    ...
    def test_crear_ventana_invalida(self, db):
        db.crear_perfil({"alias": "A", "edad": 40, "sexo": "hombre", "telegram_chat_id": "1"})
    ...
    def test_borrar_rutina_ajena_no_la_borra(self, db):
        ...
        assert len(_json(mcp.listar_rutinas_mcp(chat_id="111"))) == 1
...
class TestHoraAvisoMCP:
    def test_ver_configurar_y_desactivar(self, db):
        db.crear_perfil({"alias": "A", "edad": 40, "sexo": "hombre", "telegram_chat_id": "1"})
    ...
    def test_hora_invalida(self, db):
        db.crear_perfil({"alias": "A", "edad": 40, "sexo": "hombre", "telegram_chat_id": "1"})
...
class TestRiesgoRutinasDiaMCP:
    def test_calcula_riesgo_por_ventana_con_la_rutina(self, db, monkeypatch):
    ...
    def test_sin_ubicacion_error_claro(self, db, monkeypatch):
    ...
    def test_perfil_inexistente_error_claro(self, db, monkeypatch):
        def _no_deberia_calcular(*args, **kwargs):
            raise AssertionError("no se calcula sin perfil")

        monkeypatch.setattr(mcp, "_try_prediction", _no_deberia_calcular)
        out = _json(mcp.riesgo_rutinas_dia_mcp(alias="NoExiste"))
        assert out.get("error") and "NoExiste" in out["error"]

    def test_weekday_invalido(self, db):
    ...
    def test_sin_rutinas_ese_dia(self, db, monkeypatch):
        db.crear_perfil({"alias": "A", "edad": 40, "sexo": "hombre",
                         "lat": 40.4, "lon": -3.7, "provincia": "Madrid",
                         "telegram_chat_id": "1"})
