...
Al cerrar una feature, `finish` commitea automáticamente **solo cuando recibe
el flag explícito `--commit`**, el que pasa el lider al cerrar. Sin el flag el
comportamiento es el de hoy (GIT-001): propone el mensaje en
`data.commit_suggestion` y NO commitea — ningún otro agente ni asistente
commitea por su cuenta. Con el flag, el commit se acota a `--changes` + los
ficheros del cierre y se avisa (sin commitear) si no queda nada que commitear:

```bash
uv run python -m agents --json run harness finish --id DATA-001 \
  --evidence "$(make test 2>&1 | tail -5)" \
  --changes "climasafeai/features/x.py;climasafeai/models/y.py" --commit
```
...
| `run harness finish` | `--id`, `--evidence`, `--changes`, `--decisions`, `--pending`, `--commit` |
